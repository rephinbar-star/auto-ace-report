...
import { Loader2, Search, Car, Link as LinkIcon, CheckCircle } from "lucide-react";
...
interface VehicleInputStepProps {
  onComplete: (vehicle: VehicleInfo, listingUrl?: string) => void;
...
export function VehicleInputStep({ onComplete, initialData }: VehicleInputStepProps) {
  ...
  const handleListingSubmit = async (data: z.infer<typeof listingSchema>) => {
    setIsLoading(true);
    // For now, we'll just pass through - Firecrawl integration will be added later
    toast({
      title: "Feature Coming Soon",
      description: "Listing import will be available after Firecrawl is connected. Please use VIN or manual entry for now.",
    });
    setIsLoading(false);
  };
