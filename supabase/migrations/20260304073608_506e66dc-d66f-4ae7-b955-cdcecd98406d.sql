...
DROP FUNCTION IF EXISTS public.get_marketplace_listings(text,text,integer,integer,text,text,numeric,numeric,integer,text,integer,integer);
...
RETURNS TABLE (
  ...
  fetched_at timestamptz, user_id uuid, created_at timestamptz, status text,
  source text, external_id text, condition text, make text, model text, "trim" text,
  fetched_for_zip text, zip_code text, city text, state text, vin text,
  images text[], listing_url text, seller_type text, seller_name text,
  body_style text, fuel_type text, transmission text, drivetrain text,
  exterior_color text, description text
)
LANGUAGE sql
VOLATILE
SECURITY DEFINER
...
AS $$
  ...
  FROM (
    SELECT *,
      ROW_NUMBER() OVER (PARTITION BY COALESCE(seller_name, id::text) ORDER BY RANDOM()) AS rn
